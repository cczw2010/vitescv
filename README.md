基于`vite`,`vu2.7`的 vue单文件组件开发模式渲染小脚手架(CSR)， 支持自定义模块,支持自动组件加载.

`vitescv@1.x` 基于 `vite3`
`vitescv@2.x` 基于 `vite5`
...
page404:'/404',                 //404页面
```
`2.1.3`开始配置文件支持function,方便定制：
```
# config.js
export default function({ command, mode, ssrBuild }){
	return{
		...
    modules:[
      "@vitescv/mock":mode==='development'?{}:false,
    ]
  	...
  }
}
...
`2.1.3`开始参数设置为`false`，模块将不被加载和编译
...
[@vitescv/axios](./packages/axios)

[@vitescv/elementui](./packages/elementui)

[@vitescv/vuetify](./packages/vuetify)

[@vitescv/mock](./packages/mock)

[@vitescv/auth](./packages/auth)
